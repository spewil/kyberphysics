...
dynamics, decoder = genmaps.generate_dynamics_and_mapping(
    num_channels=64,
...
with osc_server.BlockingOSCUDPServer(("127.0.0.1", 5006),
    ...
    for i, target_idx in zip(range(num_targets), random_targets):
        trial_emg_filename = record_path + "emg__direction_trial_" + str(
            i) + "__"
        trial_behavior_filename = record_path + "behavior__direction_trial_" + str(
            i) + "__"
